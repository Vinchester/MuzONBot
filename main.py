...
def get_text_messages(message):
    ...
    btn7 = types.KeyboardButton(text="/Гитарное оборудование")
...
@bot.message_handler(commands=["Гитарное оборудование"])
def get_text_messages(message):
    keyboard = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
    btn1 = types.KeyboardButton(text="/Струны для гитар")
...
@bot.message_handler(commands=["Струны для гитар"])
def get_text_messages(message):
    keyboard = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
...
def get_text_messages(message):
    ...
    if message.text == "Струны для акустической гитары":
        ...
        photo = open('Strings/Blackhawk-8011.jpgack', 'rb')
        bot.send_photo(message.from_user.id, photo)
